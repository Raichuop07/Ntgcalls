...
from config import MUSIC_BOT_NAME, adminlist
    ...
    async def wrapper(client, message):
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(f"» {MUSIC_BOT_NAME} ɪs ᴜɴᴅᴇʀ ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ ʙᴇᴄᴀᴜsᴇ ᴏғ sᴏᴍᴇ ᴜᴘᴅᴀᴛᴇs, sᴏʀʀʏ ғᴏʀ ᴛʜᴇ ɪɴᴄᴏɴᴠᴇɴɪᴇɴᴄᴇ.")
        try:
...
def ActualAdminCB(mystic):
    async def wrapper(client, CallbackQuery):
        if (await is_maintenance() is False) and (CallbackQuery.from_user.id not in SUDOERS):
            return await CallbackQuery.answer(f"» {MUSIC_BOT_NAME} ɪs ᴜɴᴅᴇʀ ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ ʙᴇᴄᴀᴜsᴇ ᴏғ sᴏᴍᴇ ᴜᴘᴅᴀᴛᴇs, sᴏʀʀʏ ғᴏʀ ᴛʜᴇ ɪɴᴄᴏɴᴠᴇɴɪᴇɴᴄᴇ.", show_alert=True)
        try:
