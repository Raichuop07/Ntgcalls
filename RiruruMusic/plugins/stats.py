...
from RiruruMusic.utils.inline.stats import back_stats_buttons, stats_buttons
from RiruruMusic.utils.database import get_served_chats, get_served_users, get_sudoers
...
async def back_buttons(client, CallbackQuery: CallbackQuery, _):
    ...
    upl = stats_buttons(_)
    try:
        await CallbackQuery.edit_message_caption(_["gstats_2"].format(config.MUSIC_BOT_NAME), reply_markup=upl)
    except:
