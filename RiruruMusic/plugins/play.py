...
from RiruruMusic import Spotify, Telegram, YouTube, app
...
from RiruruMusic.utils.inline.play import (
    livestream_markup, playlist_markup, slider_markup, track_markup, botplaylist_markup
)
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            if "playlist" in url:
                try:
                    details = await YouTube.playlist(url, config.PLAYLIST_FETCH_LIMIT)
                except Exception as e:
                    print(e)
            ...
            try:
                ...
                await mystic.edit_text("ᴛʜᴇʀᴇ's ᴀɴ ᴇʀʀᴏʀ ɪɴ ᴛʜᴇ ʙᴏᴛ, ᴩʟᴇᴀsᴇ ʀᴇᴩᴏʀᴛ ɪᴛ ᴛᴏ sᴜᴩᴩᴏʀᴛ ᴄʜᴀᴛ ᴀs sᴏᴏɴ ᴀs ᴩᴏssɪʙʟᴇ.")
        ...
        if plist_type:
            ...
            if slider:
                ...
                await message.reply_photo(
                    photo=details["thumb"],
                    caption=_["play_11"].format(details["title"].title(), details["duration_min"]),
...
@app.on_callback_query(filters.regex("AltPlaylists") & ~BANNED_USERS)
@languageCB
async def play_playlists_command(client, CallbackQuery, _):
    ...
    if ptype == "yt":
        spotify = False
        try:
            result = await YouTube.playlist(videoid, config.PLAYLIST_FETCH_LIMIT, True)
        except Exception:
            return await mystic.edit_text(_["play_3"])
    if ptype == "spplay":
        try:
...
async def slider_queries(client, CallbackQuery, _):
    ...
    if what == "F":
        ...
        try:
            await CallbackQuery.answer(_["playcb_2"], show_alert=True)
        except:
            pass
